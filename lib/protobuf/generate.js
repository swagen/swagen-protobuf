...
class Generator {
    ...
    getOperationSignature(operationName, operation) {
        ...
        const methodSig =  returnType=='any'? `rpc ${operationName}(${parameter}) returns {}`:`rpc ${operationName}(${parameter}) returns {(${returnType.complex})}`;
    ...
    getReturnType(operation) {
        ...
        for (const statusKey in operation.responses) {
            ...
            if (statusCode >= 200 && statusCode < 300 && operation.responses[statusKey].dataType) {
                return operation.responses[statusKey].dataType;
            }
        }
